function Skills() {
    ...
    return (
        <div id="skills" className="flex flex-col w-[60%] items-center min-h-screen px-[100px] py-[60px] justify-center snap-center">
            <div className="flex flex-col gap-3">
                <p>Technical</p>
                <div className="flex flex-wrap p-3 gap-x-3 gap-y-4 justify-start bg-neutral-bg1/40 rounded-sm">
                    {skills['Technical'].map((s) => SkillBlock(s))}
                </div>
                <p>Frameworks & Libraries</p>
                <div className="flex flex-wrap p-3 gap-x-3 gap-y-4 justify-start bg-neutral-bg1/40 rounded-sm">
                    {skills['FL'].map((s) => SkillBlock(s))}
                </div>
                <p>Design</p>
                <div className="flex flex-wrap p-3 gap-x-3 gap-y-4 justify-start bg-neutral-bg1/40 rounded-sm">
...
function SkillBlock(s) {
    return (<div className="flex items-center gap-2 min-w-20 p-2 bg-neutral-bg1 rounded-sm"><img src={'/icons/' + s.logokey + '.svg'} alt={s.name} className="size-6 "/><p>{s.name}</p></div>)
}
